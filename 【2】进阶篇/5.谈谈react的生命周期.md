...
```mermaid
graph TD
    A[React生命周期] --> B[挂载阶段 Mounting]
    A --> C[更新阶段 Updating]
    A --> D[卸载阶段 Unmounting]
    A --> E[错误处理 Error Handling]
    
    B --> B1[constructor]
    B --> B2[getDerivedStateFromProps]
    B --> B3[render]
    B --> B4[componentDidMount]
    
    C --> C1[getDerivedStateFromProps]
    C --> C2[shouldComponentUpdate]
    C --> C3[render]
    C --> C4[getSnapshotBeforeUpdate]
    C --> C5[componentDidUpdate]
    
    D --> D1[componentWillUnmount]
    
    E --> E1[getDerivedStateFromError]
    E --> E2[componentDidCatch]
```
